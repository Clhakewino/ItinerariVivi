...
export default function Navbar() {
  ...
  return (
    <>
<>
  {/* 1. Logo: 'absolute' per stare sopra lo sfondo, 'z-40' per non farsi coprire */}
  <div className="absolute top-0 left-0 w-full px-8 py-4 flex justify-start items-center bg-transparent pointer-events-none z-40">
    <Link 
      href="/" 
      className="text-2xl font-black text-rose-500 tracking-tighter drop-shadow-[2px_2px_1.5px_rgba(0,0,0,0.4)] pointer-events-auto"
    >
      ItinerariVivi
    </Link>
  </div>

  {/* 2. Bottone Cerca: 'fixed' e 'z-50' (più alto del logo) per restare sempre sopra tutto */}
  <div className={`fixed top-4 right-8 z-50 transition-all duration-500 ${showSearchIcon ? 'opacity-90 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>
    <button 
      onClick={() => setIsSearchOpen(true)} 
      className="bg-rose-600 text-white/80 p-2 rounded-full shadow-lg active:scale-95 transition"
    >
